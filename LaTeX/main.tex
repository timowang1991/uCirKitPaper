...
\newcommand{\papertitle}{CircuitStack}

% End of preamble. Here it comes the document.
\begin{document}

\title{\papertitle: Supporting Rapid Prototyping\\and Evolution of Electronic Circuits}

\teaser{
\centering \includegraphics[width=1.0\textwidth]{figures/Figure1.pdf} 
\caption{(A) Exploded view.
         (B) Assembled state.
         (C) Zones marked with same color are electrically connected via board contact when assembled, and all current is conducted to the top layer.
         (D) Each spring that is connected by PCP allows for conductivity between the springs.
         (E) Connections between springs are highlighted by the red wire.
         (F) Contact springs (part number OG-320816) used on our prototype.}
\label{fig:figure1}
}
...
\keywords{Rapid Prototyping; Breadboard; Printable Circuits; Conductive Ink}

\input{02-Introduction.tex}
%\input{03-Related_Work.tex}
\input{04-System_Implementation.tex}
% \input{05-User_Study.tex}
\input{05-User_Study_v2.tex}
\input{06-Results.tex}
\input{07-Limitation.tex}
%\input{08-Future_Work.tex}
